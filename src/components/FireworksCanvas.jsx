  ...
  switch (type) {
    ...
    case "burst": {
      for (let i = 0; i < count; i++) {
        ...
        particles.push(new Particle(x, y, Math.random() > 0.5 ? color : color2,
          { x: Math.cos(angle) * speed, y: Math.sin(angle) * speed },
    ...
    case "star": {
      ...
      for (let i = 0; i < count; i++) {
        ...
        particles.push(new Particle(x, y, i % 3 === 0 ? "#ffffff" : color,
          { x: Math.cos(angle) * speed, y: Math.sin(angle) * speed },
...
const FireworksCanvas = () => {
  ...
  useEffect(() => {
    ...
    const particleCap = width < 1024 ? MAX_PARTICLES_MOBILE : MAX_PARTICLES;
    canvas.width = width;
