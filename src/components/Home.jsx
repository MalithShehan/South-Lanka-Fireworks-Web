...
const Home = () => {
    ...
    const handleScroll = () => {
      ...
      if (!ticking) {
        ...
        rafRef.current = requestAnimationFrame(() => {
          ...
          if (parallaxRef.current) parallaxRef.current.style.transform = `translateY(${y * 0.3}px)`;
          if (overlayRef.current) overlayRef.current.style.opacity = String(Math.max(0, 1 - y / 700));
          ticking = false;
        });
      }
    };
  ...
  return (
    <div id="home" className="relative w-full overflow-hidden" style={{ minHeight: "100dvh" }} role="banner">
      ...
      <motion.div
        ref={overlayRef}
        className="relative z-30 flex flex-col items-center justify-center text-center min-h-screen px-4"
      ...
      >
        ...
        <motion.div variants={itemVariants} className="mb-6 min-h-[40px]">
          <TypewriterText
            texts={HERO_MESSAGES}
            typingSpeed={70}
            deletingSpeed={35}
        ...
        <motion.h1
          ...
          style={{ textShadow: "0 2px 10px rgba(0,0,0,0.5),0 0 20px rgba(251,191,36,0.12)", filter: "drop-shadow(0 2px 8px rgba(0,0,0,0.4))" }}
        >
        ...
        >
          South Lanka Fireworks delivers unforgettable fireworks shows for weddings, festivals, and corporate events across Sri Lanka.
        </motion.p>
