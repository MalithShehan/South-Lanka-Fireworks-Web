...
const QUICK_ACTIONS = [
  { label: "Call", helper: "Contact", href: "tel:+94777135516", Icon: FiPhoneCall },
  { label: "WhatsApp", helper: "Message", href: "https://wa.me/94777135516", Icon: FiMessageCircle },
];
...
const Navbar = () => {
  ...
  const handleNavClick = (to) => {
    ...
    if (titles[to]) document.title = titles[to];
  };
    ...
    >
      ...
      <div className="relative max-w-[1200px] mx-auto flex flex-nowrap items-center justify-between gap-3 sm:gap-4 md:gap-6 px-4 sm:px-6 md:px-10 py-2 text-gray-200">
        ...
        <button
          ...
          className="group flex min-w-0 items-center gap-3 text-xl sm:text-2xl font-bold tracking-wide cursor-pointer bg-transparent border-none"
            ...
            {MENU_ITEMS.map(({ name, to }) => (
              <li key={to}>
                <button
                  ...
                  className="group relative px-2 py-1 tracking-[0.08em] text-white hover:text-white transition bg-transparent border-none cursor-pointer"
                >
      ...
      {navOpen && (
        <>
          <div onClick={closeNav} className="fixed inset-0 bg-black/60 z-40 lg:hidden animate-[fadeIn_0.3s_ease]" />
          <div
            ref={sidebarRef}
            className="fixed top-0 left-0 h-screen w-3/4 sm:w-2/5 text-white z-50 shadow-2xl flex flex-col backdrop-blur-xl border-r border-white/10 animate-[slideInLeft_0.4s_ease]"
            style={{ background: "linear-gradient(180deg, rgba(18,11,8,0.98) 0%, rgba(23,16,13,0.96) 50%, rgba(13,9,7,0.98) 100%)" }}
          >
            <div className="flex items-center justify-between p-5 border-b border-white/10 gap-3">
              <button
                onClick={() => handleNavClick("home")}
                className="flex items-center gap-2 cursor-pointer min-w-0 flex-1 bg-transparent border-none"
              >
                <img
                  src={asset("/assets/SouthLankaFireworks.webp")}
                  alt="South Lanka Fireworks Logo"
                  ...
                  decoding="async"
                />
                <span className="bg-gradient-to-r from-yellow-300 via-orange-300 to-rose-300 bg-clip-text text-transparent font-bold font-kaushan text-base tracking-wide truncate">
                  South Lanka
                </span>
              </button>
              <button
            ...
            <div className="p-6 border-b border-white/5 bg-white/5">
              <p className="text-xs uppercase tracking-[0.4em] text-white/60 mb-3">Rapid Response</p>
            ...
            <ul className="flex flex-col p-6 space-y-3 overflow-y-auto">
              {MENU_ITEMS.map(({ name, to, helper }, index) => (
                ...
                >
                  <button
                    onClick={() => handleNavClick(to)}
                    className="w-full flex items-center gap-4 rounded-2xl border border-white/10 bg-white/0 px-4 py-4 text-white transition-all duration-300 hover:border-amber-200/30 hover:bg-white/10 bg-transparent"
                  >
                    <span className="text-xs font-semibold tracking-[0.4em] text-white/50">
